 ...
 *
 * Copyright (C) 2012-2016 Texas Instruments
 *
...
/* For 1R3/1R4 ALE h/w */
#define ALE_ENTRY_PORT_MASK_BITS_1R3_R4		9
#define ALE_ENTRY_PORT_NUM_BITS_1R3_R4		4
#define ALE_ENTRY_VLAN_FIELD_BITS_1R3_R4	5
#define ALE_UNKNOWN_VLAN_FIELD_BITS_1R3_R4	5
/* default values in other devices */
#define ALE_ENTRY_PORT_MASK_BITS	3
#define ALE_ENTRY_PORT_NUM_BITS		2
#define ALE_ENTRY_VLAN_FIELD_BITS	3
				 ...
				 int port_mask)
{
	...
	port = cpsw_ale_get_port_num(ale_entry, ale->port_num_bits);
...
int cpsw_ale_control_set(struct cpsw_ale *ale, int port, int control,
			 int value)
{
	int offset, shift, max_control = ALE_NUM_CONTROLS;
	const struct ale_control_info *info;
	u32 tmp, mask;

	/* ALE version 1.4 has 4 more controls */
	if (ale->revision != ALE_VERSION_1R4)
		max_control -= 4;

	if (control < 0 || control >= max_control)
		return -EINVAL;
...
int cpsw_ale_table_store_vlan(struct cpsw_ale *ale, const u8 *params_str,
			      size_t str_len)
{
	int ret;
...
int cpsw_ale_table_store_vlan_ucast(struct cpsw_ale *ale, const u8 *params_str,
...
int cpsw_ale_table_store_u_proc(struct cpsw_ale *ale, const u8 *params_str,
				size_t str_len)
{
	return  cpsw_ale_table_store_vlan_ucast(ale, params_str, str_len, 0);
}

int cpsw_ale_table_store_vu_proc(struct cpsw_ale *ale, const u8 *params_str,
				 size_t str_len)
{
	return  cpsw_ale_table_store_vlan_ucast(ale, params_str, str_len, 1);
}

int cpsw_ale_table_store_vlan_mcast(struct cpsw_ale *ale, const u8 *params_str,
				    size_t str_len, int has_vid)
{
	int ret;
...
int cpsw_ale_table_store_m_proc(struct cpsw_ale *ale, const u8 *params_str,
				size_t str_len)
{
	return  cpsw_ale_table_store_vlan_mcast(ale, params_str, str_len, 0);
}

int cpsw_ale_table_store_vm_proc(struct cpsw_ale *ale, const u8 *params_str,
				 size_t str_len)
{
...
int cpsw_ale_table_store_oui(struct cpsw_ale *ale, const u8 *params_str,
			     size_t str_len)
{
	int ret;
...
void cpsw_ale_start(struct cpsw_ale *ale)
{
	...
	/* version specific masks */
	if ((ale->revision == ALE_VERSION_1R3) ||
	    (ale->revision == ALE_VERSION_1R4) ||
	    (ale->revision == ALE_VERSION_9R3)) {
		ale->port_mask_bits = ALE_ENTRY_PORT_MASK_BITS_1R3_R4;
		ale->port_num_bits = ALE_ENTRY_PORT_NUM_BITS_1R3_R4;
		ale->vlan_field_bits = ALE_ENTRY_VLAN_FIELD_BITS_1R3_R4;
		/* override the unknown vlan masks */
		ale_controls[ALE_PORT_UNKNOWN_VLAN_MEMBER].bits =
				ALE_UNKNOWN_VLAN_FIELD_BITS_1R3_R4;
		...
		ale->port_mask_bits = ALE_ENTRY_PORT_MASK_BITS;
		ale->port_num_bits = ALE_ENTRY_PORT_NUM_BITS;
		ale->vlan_field_bits = ALE_ENTRY_VLAN_FIELD_BITS;
	}
