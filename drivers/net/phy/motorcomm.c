...
static int yt8511_led_init(struct phy_device *phydev)
{
	...
	val = ytphy_read_ext(phydev, YT8511_EXTREG_LED2);
	if (val < 0)
		return val;

	val = 0x00;

	ret = ytphy_write_ext(phydev, YT8511_EXTREG_LED2, val);
	if (ret < 0)
		return ret;

	val = ytphy_read_ext(phydev, YT8511_EXTREG_LED3);
	if (val < 0)
		return val;

	val = 0x20;

	ret = ytphy_write_ext(phydev, YT8511_EXTREG_LED3, val);
